...
import { initDatabase, semanticSearch, advancedSearch, getSearchStats } from './vector-search.js'
...
async function runExamples() {
  ...
  try {
    ...
    console.log(`Average chunks per speech: ${stats.avgChunksPerSpeech || 'N/A'}`)
    ...
    const question = "What are the main concerns about artificial intelligence mentioned in UN speeches?"
    ...
    ragResult.sources.forEach(source => {
      console.log(`  • ${source.country}, ${source.year} (${source.speaker}) - Distance: ${source.distance}`)
    })
    ...
    const comparison = await compareperspectives(db, topic, countries, [], { searchLimit: 2 })
    
    console.log(`\n${comparison.summary}`)
    comparison.perspectives.forEach((perspective, i) => {
      console.log(`\n--- ${perspective.entity} ---`)
      console.log(`Sources: ${perspective.sources} (Years: ${perspective.sample_years.join(', ')})`)
      console.log(perspective.perspective.slice(0, 300) + (perspective.perspective.length > 300 ? '...' : ''))
    })
    ...
    if (sampleChunk) {
      console.log(`Finding content similar to chunk from ${sampleChunk.country}, ${sampleChunk.year}:`)
